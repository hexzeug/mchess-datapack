$execute unless entity @e[tag=mchess.pointer,nbt={data:{player:$(UUID)}}] run summon interaction ~ ~ ~ {Tags:["mchess","mchess.pointer"],data:{player:$(UUID)},width:2f,height:2f,response:true}
